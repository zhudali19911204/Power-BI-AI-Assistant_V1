...
| 1.1 | 2026-07-13 | 固定 Modeling MCP 0.5.0-beta.11 只读契约，并增加预览软件许可与安装包分发门禁 |
...
- [Power BI Modeling MCP Windows x64 npm 包](https://www.npmjs.com/package/@microsoft/powerbi-modeling-mcp-win32-x64)
...
| NFR-010 | 可安装性 | 在第三方许可门禁通过的前提下，测试安装包内置运行时，最终用户无需安装 Node.js 或 .NET |
...
交付：两类专用完整性规则、生成、验证、风险提示、确认写入；许可门禁通过后生成第一个 Windows 测试安装包。

退出条件：计算列与计算表均完成澄清、生成、验证、写入和回滚；同名对象保护有效；Microsoft 预览组件分发已取得另行授权或法务确认后，无 Node/.NET 的测试机可运行安装包。许可门禁未通过时不得生成或分发含 MCP 二进制的安装包。
...
- 开发阶段使用 npm 脚本启动和测试。
- 阶段 1 使用本机安装的固定 Windows x64 MCP npm 依赖进行内部开发测试，不进入生产或实时运行环境。
- `@microsoft/powerbi-modeling-mcp-win32-x64@0.5.0-beta.11` 的 PREVIEW 许可禁止分享、发布、分发或转让该软件；当前构建配置必须排除该二进制。
- 只有获得 Microsoft 另行许可或完成组织法务确认后，阶段 5 才可使用 NSIS 构建含 MCP 的 Windows x64 每用户安装包。
- 许可门禁通过后的安装包内置 Electron 和固定 MCP 平台程序，并附完整许可和第三方声明；门禁未通过时必须暂停安装包交付并评估合法替代部署方式。
...
| Modeling MCP 预览许可禁止分发且限制为开发测试 | 无法按原计划内置到测试安装包或用于实时环境 | 阶段 1 仅本机开发；阶段 5 设置 Microsoft 授权/法务硬门禁，未通过时禁止打包分发 |
